...
function ttHeaderDropdown(){
  ...
  $('header').on('click', '.tt-dropdown-obj', function(e) {
    ...
    function popupObjMobile(obj){
      ...
      $('html').toggleClass('v-cart-open');
      $('header .tt-cart .tt-dropdown-menu .tt-cart-box').perfectScrollbar().addClass('perfectScrollbar');/* cart * / */
      }, 50)
    };
    function popupObjDesctop(obj){
      ...
      if ($('.tt-btn-addtocart').is(target)){
        toggleDropdown('.tt-cart');
      }
    ...
    function toggleDropdown(obj){
      /*
        - Updated to the Dropdown toggle to open the new slide-cart from the right.
      */
      if(obj.hasClass('tt-cart')) {
        //$('header').find('.tt-cart.active').removeClass('active');
        obj.toggleClass('activetoview');
        setTimeout(function(){
        obj.toggleClass('active').find('.tt-dropdown-menu').removeAttr("style");
        $body.toggleClass('tt-popup-dropdown');
          ...
          if($this.hasClass('active') && !$this.hasClass('tt-search')){
            if(ttwindowWidth <= 1024){
              ...
              $('.tt-dropdown-obj').each( function () {
                ...
                if($(this).hasClass('activetoview')){
                  $(this).removeClass('activetoview');
                }
                if($('body').hasClass('tt-popup-dropdown')){
                  $('body').removeClass('tt-popup-dropdown');
                }
                if($('html').hasClass('v-cart-open')){
                  $('html').removeClass('v-cart-open');
                }
              });
            };
          };
...
function ttProductHover() {
  $document.on('mouseenter mouseleave', '#tt-pageContent .tt-product', function(e) {
    ...
    function ttOnHover(e){
      // $this.stop().css({height: $this.innerHeight()}).addClass('hovered');
...
function ttLookbook(ttwindowWidth){
  ...
  function showMessage(_this, e){
    ...
    function closePopupDesctop(){
      ...
      if($('.tt-cart').hasClass('activetoview')){
        $('.tt-cart').removeClass('activetoview'); 
      }
    };
...
function ttInputCounter() {
  blocks.ttInputCounter.find('.minus-btn, .plus-btn').click(function( e ) {
    ...
    $(".inventory-status .inventory-status-hide").hide();
    var variant_id = $(".input_variant").val();
    var select_var = $("#var-"+variant_id);
    var inventory_quantity = (select_var).data('qty');
    var inventory_policy = (select_var).data('policy');
    var digital_item = (select_var).data('digital');
    if(digital_item == 1) {
      $(".gift-card-variant.inventory-status-hide").show();
    }
    else if(inventory_quantity >= count)
    {
      $(".available-variant.inventory-status-hide").show();
      $("button.btn-addtocart").prop('disabled', false);
    }
    else if((count > inventory_quantity) && (inventory_policy == "continue"))
    {
      $(".continue-variant.inventory-status-hide").show();
      $("button.btn-addtocart").prop('disabled', false);
    }
    else
    {
      $(".soldout-variant.inventory-status-hide").show();
      $("button.btn-addtocart").prop('disabled', true);
    }
        ...
        currentAjaxRequest = $.getJSON(searchURL + '&view=json', function(data) {
          ...
          if(data.results_count == 0) {
            ...
            $.each(data.results, function(index, item) {
              var splits = item.url.split('/');
              var handlelong = splits[splits.length - 1];
              var splits = handlelong.split('?');
              var myhandle = '/products/' + splits[0] + '.js';
              $.getJSON(myhandle, function(jsonproduct) {
                if(!jsonproduct.tags.includes('hidden')) {
...
$(document).ready(function(){
  $(document).on('click','.addtocart-item-js',function(){
    $('.tt-cart .tt-dropdown-toggle').trigger('click');
  });
  ...
  setTimeout(function()
  {
    $( ".allvariants span" ).each(function( index )
    {
      var variant_title = $( this ).data('title');
      var percentoff = $( this ).data('percentoff');
      if(percentoff != "")
      {
        var percentoffhtml = "<span class='sale-badge-outher'><span class='sale-badge'>"+percentoff+"</span></span>";
        var selector_swatch = $(".tt-options-swatch li a[data-value='"+variant_title+"']");
        $(percentoffhtml).insertBefore(selector_swatch);
      }
   ...
   // $(window).scroll(function(event) {
        ...
        //if (st > lastScroll) {
          //  $('.desctop-menu-large').addClass('scroll-down');
        //} else {
        //    $('.desctop-menu-large').removeClass('scroll-down');
        //}
       // lastScroll = st;
   // });
   ...
   (function(){
	...
	$(document).on('click','.v-collection-toolbar-top',function(){
		$("body").removeClass("filter-open")
	})
	$(document).on('click','.v-collection-toolbar-bg',function(){
		$("body").removeClass("filter-open")
	})
	$(document).on('click','.v-collection-bottom-bar a',function(){
		$("body").removeClass("filter-open")
	})
	
	
	
	$(document).on('click','.tt-search',function(){
